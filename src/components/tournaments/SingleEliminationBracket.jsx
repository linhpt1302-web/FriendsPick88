  ...
  const handleSaveScore = (e) => {
    ...
    if (selectedMatch.isFinal) {
      ...
      if (Math.max(sA, sB) < 15) {
        alert('Trận đấu loại trực tiếp phải thi đấu chạm đến 15 điểm.');
        return;
      }
      if (Math.abs(sA - sB) < 2) {
          ...
          <div key={round.roundNumber} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            >
              ...
              <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                {round.roundNumber === 3 ? 'Thắng 2/3 Set (Chạm 11)' : '1 Set (Chạm 15)'}
              </span>
            </div>
                    ...
                    <div className="flex-between" style={{ fontSize: '0.75rem', marginBottom: '0.6rem' }}>
                      ...
                      {isCompleted ? (
                        <span style={{ color: '#34d399', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '0.2rem' }}>
                          <CheckCircle2 size={12} /> Đã Đấu Xong
                        </span>
                      ) : isReady ? (
          ...
          <form id="bracket-score-form" onSubmit={handleSaveScore}>
            <div style={{ textAlign: 'center', marginBottom: '1.25rem' }}>
              ...
              <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                {selectedMatch.isFinal ? 'Chung Kết Tranh Cúp (Thắng 2/3 Set chạm 11)' : 'Trận Loại Trực Tiếp (1 Set chạm 15 điểm)'}
              </span>
            </div>
            ...
            {selectedMatch.isFinal ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '1rem' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span style={{ fontWeight: '700', fontSize: '0.85rem' }}>Set 3 (Nếu cần):</span>
                  <input
                    type="number"
                    min="0"
                    max="25"
